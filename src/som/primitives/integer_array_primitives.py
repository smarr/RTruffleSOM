...
from som.vmobjects.integer_array import IntegerArray
...
def _at(ivkbl, rcvr, args):
    i = args[0]
    return  rcvr.get_idx(i.get_embedded_integer() - 1)
...
def _atPut(ivkbl, rcvr, args):
    ...
    rcvr.set_idx(index.get_embedded_integer() - 1, value)
    return value
...
def _new(ivkbl, rcvr, args):
    length = args[0]
    return IntegerArray(length.get_embedded_integer())
...
def _do(ivkbl, rcvr, args):
    ...
    while i < length:  # the array itself is zero indexed
        ...
        block_method.invoke(block, [rcvr.get_idx(i)])
        i += 1
...
def _putAllWithBlock(rcvr, block):
    ...
    arr = rcvr.get_int_array()
    i = 0
    length = rcvr.get_size()
    while i < length:  # the array itself is zero indexed
        put_all_driver.jit_merge_point(block_method = block_method)
        arr[i] = block_method.invoke(block, []).get_embedded_integer()
        i += 1
    return rcvr
...
def _putAll(ivkbl, rcvr, args):
    ...
    for i, _ in enumerate(arr):
        arr[i] = arg.get_embedded_integer()
    return rcvr


class IntegerArrayPrimitives(Primitives):
